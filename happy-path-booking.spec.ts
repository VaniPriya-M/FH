    ...
    */
    test('sign in → select scan → schedule → pay → confirm booking', async ({
        ...
        await test.step('Complete payment with test card', async () => {
            ...
            // Use Stripe test card details from TestData (includes ZIP code)
            await paymentPage.fillCardDetails(TestData.validCard);
            // Click the Continue button to submit payment
            await paymentPage.submitPayment();
        });

        // Step 6: Verify booking confirmation
        await test.step('Verify booking is confirmed', async () => {
            // Wait for the confirmation page (URL contains 'scan-confirm')
            await page.waitForURL(
                (url) => url.pathname.toLowerCase().includes('scan-confirm'),
                { timeout: 60_000 },
            );

            // Grab the full page text to verify confirmation details
